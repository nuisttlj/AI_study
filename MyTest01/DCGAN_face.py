    ...
    def forward(self, x, reuse=False):
        with tf.variable_scope("d_net", reuse=reuse):
            with slim.arg_scope([slim.conv2d], padding="SAME",
                ...
                net = slim.conv2d(net, 256, 5, 2)
                net = tf.nn.leaky_relu(slim.batch_norm(net, decay=0.9, epsilon=1e-5))
                net = slim.conv2d(net, 512, 5, 2)
                net = tf.nn.leaky_relu(slim.batch_norm(net, decay=0.9, epsilon=1e-5))
                net = slim.fully_connected(tf.reshape(net, [128, 6 * 6 * 512]), 1)
...
class G_Net:
    def forward(self, x):
        with tf.variable_scope("g_net"):
            net = slim.fully_connected(x, 6 * 6 * 512)
            net = tf.nn.relu(slim.batch_norm(tf.reshape(net, [-1, 6, 6, 512]), decay=0.9, epsilon=1e-5))
            with slim.arg_scope([slim.conv2d_transpose],
                ...
                net = tf.nn.relu(slim.batch_norm(net, decay=0.9, epsilon=1e-5))
                net = slim.conv2d_transpose(net, 3, 5, 2, activation_fn=tf.nn.tanh, normalizer_fn=None)
                net = tf.nn.tanh(net)
    ...
    with tf.Session() as sess:
        ...
        for i in range(100000):
            ...
            if i % 10 == 0:
                ...
                # exit()
                img_array = np.array(g_out_)[0] / 2 + 0.5
