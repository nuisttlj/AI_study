...
dev_save_path = r"E:\mywiderface_dev"

for face_size in [12, 24, 48]:
    print("gen {} image".format(face_size))
    positive_image_dir = os.path.join(dev_save_path, str(face_size), "positive")
    negative_image_dir = os.path.join(dev_save_path, str(face_size), "negative")
    part_image_dir = os.path.join(dev_save_path, str(face_size), "part")

    for dir_path in [positive_image_dir, negative_image_dir, part_image_dir]:
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)

    positive_anno_filename = os.path.join(dev_save_path, str(face_size), "positive.txt")
    negative_anno_filename = os.path.join(dev_save_path, str(face_size), "negative.txt")
    part_anno_filename = os.path.join(dev_save_path, str(face_size), "part.txt")
    ...
    try:
                    ...
                    with Image.open(image_file) as img:
                        ...
                        for box in boxes:
                            ...
                            for _ in range(1):
                                ...
                                side_len = np.random.randint(face_size, int(max(min(img_w / 4, img_h / 4), min(w, h))))
        ...
        # 单独生成手部负样本
        nonface_source_path = r"D:/hand_image_dev/"
